...
static const char* const password_chars = "123456789ABCDEFGHJKLMNPQRSTUVWXYZ";

uint16_t output_vals[6];
...
int find_multiplier(char* system_in)
{
    ...
    const char* hex_values_ptr = hex_values;
...
int main(int argc, char* argv[])
{
    ...
    if (len != 12) {
        fprintf(stderr, "Mac address must be exactly 12 characters!");
    ...
    if (len != 6) {
        fprintf(stderr, "McDonalds 'Store Number' must be exactly 6 digits long!");
    ...
    if (len != 6) {
        fprintf(stderr, "McDonalds 'Store Management Number of DS Card' must be exactly 6 digits long!");
